...
import { cmdPhase, spaBookingOpen, spaCustomer, state } from "../state.js";
...
function pastPanel(open) {
  ...
  F.spa.appointments.past.forEach(function (a) { panel.appendChild(apptRow(a, { bookAgain: open })); });
  return panel;
}
...
export function SpaAppointments() {
  ...
  var next = (sc === "empty" || sc === "no-history" || state.view === "empty") ? null : (F.spa.appointments.nextVariants[sc] || F.spa.appointments.nextVariants.salon);
  ...
  var page = h("section", { "class": "page", "data-route": "orders.list", "data-state": state.view, "data-visual-id": "spa-appointments", "data-capability": "target-appointments", "data-booking": open ? "open" : "closed", "data-screen-label": "Appointments (target)" });
  ...
  if (!next) {
    ...
    if (sc === "no-history") {
      ...
      left.appendChild(emptyCard);
      left.appendChild(pastPanel(open));
    }
  } else {
    ...
    hero.appendChild(h("div", { "class": "appt-hero__ref", "data-bind": "appointment.reference" }, [
      ...
      F.spaCommerce.purchaseByAppointment[next.id]
    ...
    if (cancelled) {
      ...
      hero.appendChild(h("div", { "class": "appt-hero__actions" }, [
        ActionButton({ variant: "btn--ghost", label: "Reschedule", action: "appointment.reschedule", id: next.id, disabled: phase === "pending" || phase === "conflict", visualId: "appt-reschedule" }),
        ActionButton({ variant: "btn--ghost", label: "Cancel visit", action: "appointment.cancel", id: next.id, confirm: true, pending: phase === "pending", pendingLabel: "Cancelling\u2026", disabled: phase === "conflict", visualId: "appt-cancel" })
      ]));
    } else {
    ...
    F.spa.appointments.upcoming.forEach(function (a) { upcoming.appendChild(apptRow(a)); });
