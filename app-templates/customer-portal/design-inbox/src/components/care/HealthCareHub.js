// customer-portal-design/src/components/care/HealthCareHub.js — Wave 9 (Health): appointments, plan milestones, tasks, secure documents, care team.
...
function taskDone(t) { return (t.id in state.careTasksDone) ? state.careTasksDone[t.id] : !!t.done; }

export function HealthCareHub(m) {
  ...
  var doneCount = m.tasks.filter(taskDone).length;
  ...
  var provider = h("div", { "class": "card card--pad", "data-module": "provider-card", "data-visual-id": "provider-card", "data-provider-id": p.id }, [
    ...
    h("div", { style: "display:flex;flex-direction:column;gap:8px;margin-top:14px" }, [
      ActionButton({ variant: "btn--primary", label: "Message the care team", action: "care.contactProvider", id: p.id, block: true, visualId: "care-contact-provider" }),
  ...
  m.docs.forEach(function (d) {
    docs.appendChild(h("div", { "class": "log-row", "data-module": "secure-document-row", "data-document-id": d.id }, [
      ...
      h("div", { "class": "link-action", "data-action": "care.openSecureDoc", "data-id": d.id }, "Open \u203a")
    ]));
  });
