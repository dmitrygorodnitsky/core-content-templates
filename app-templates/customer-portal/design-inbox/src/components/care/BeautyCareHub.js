// customer-portal-design/src/components/care/BeautyCareHub.js — Wave 9 (Beauty): appointments & packages, specialist preference, routine history, loyalty, products.
...
export function BeautyCareHub(m) {
  ...
  var activeId = state.careSpecialist || m.preferredId;
  ...
  var vproducts = (F.themes[state.theme] || {}).products || [];
  var recs = m.productRecs.names.map(function (n) { return vproducts.find(function (p) { return p.name === n; }); }).filter(Boolean);
