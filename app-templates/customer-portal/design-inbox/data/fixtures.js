...
(function () {
  ...
  var spa = {
    ...
    /* wave 17.1 — `media` is the BACKEND-supplied representative thumbnail for the
       row (retail -> product primary, service -> service image, package/series ->
       package or ProductModel hero). OPTIONAL and never inferred from the title,
       type, amount, filename or first order line; a multi-line order is never a
       collage. url = a public / approved asset (absent here -> the neutral
       no-media fallback renders in the same footprint); alt = backend alt text. */
    stagingOrders: [
  ...
  var spaCommerce = {
    ...
    /* ============================================================
       WAVE 17 — Product models, media galleries & published reviews.
       Enrichments layered on the retail catalog, merged by the
       IMMUTABLE product code / opaque product ref. Each is OPTIONAL:
       a failed models / reviews / media fetch never blanks the
       sellable product page — the affected region shows its own honest
       unavailable / empty / error treatment (state.spaModels /
       state.spaReviews), and the page never falls back to a guessed
       collection, a duplicated image or an invented review. Opaque refs
       (prd-*, pmd-*, med-*, rev-*) are stable non-sequential handles,
       NEVER raw Core / Media / Review / User ids. A raw Media id is
       never used as an image URL: media.url is a public / approved
       asset URL, and while it is missing the accepted striped media
       slot renders with the backend alt text. No rating average, no
       stock / bestseller / discount / crossed-out price / before-after
       claim, and no review count beyond the visible loaded set. */
    productCatalog: {
      /* opaque product ref <-> immutable PIM code (the merge key) */
      codeToRef: { "rtl-beauty-01": "prd-7f3a91", "rtl-beauty-02": "prd-2c88de", "rtl-beauty-03": "prd-b41f07", "rtl-beauty-04": "prd-9d20c5", "rtl-beauty-05": "prd-5e6a12", "rtl-beauty-06": "prd-c130fb" },
      /* ProductModel enrichment: groups the products of ONE collection / series
         and DECLARES the product-type attributes that distinguish its variants
         (model.variants). A product with no model is NEVER inferred into a
         series from its code or name — it falls under the neutral "Other
         products" group. Ordered as returned. model.media is OPTIONAL and
      ...
      /* Product read models keyed by opaque ref. media[] is in EXPLICIT backend
         order and never padded with a duplicated image. `collection` is the
         product's model membership (an enrichment — hidden while models are
         unavailable). `variantFacts` are source-provided compact facts (format,
         size, …), never inferred. `prd-longform` is a preview-only scenario for
         the long-name / long-description / long-collection overflow check
         (reachable through the dev `pdet` select, not shown as a shop card). */
      byRef: {
        "prd-7f3a91": { ref: "prd-7f3a91", code: "rtl-beauty-01", name: "Silk Repair Set", displayPrice: "$64.00", collection: { ref: "pmd-bond01", name: "Bond Repair system" },
        ...
        "prd-2c88de": { ref: "prd-2c88de", code: "rtl-beauty-02", name: "Heat Shield Spray", displayPrice: "$28.00", collection: { ref: "pmd-bond01", name: "Bond Repair system" },
          ...
          media: [
            { ref: "med-02a", url: null, alt: "Heat Shield Spray bottle, front" }
          ] },
        "prd-b41f07": { ref: "prd-b41f07", code: "rtl-beauty-03", name: "Cuticle Care Kit", displayPrice: "$22.00", collection: { ref: "pmd-nail01", name: "At-home nail care" },
          ...
          variantFacts: [{ label: "Kit", value: "Oil · pusher · balm" }],
          media: [] },
        "prd-9d20c5": { ref: "prd-9d20c5", code: "rtl-beauty-04", name: "Gel Removal Kit", displayPrice: "$18.00", collection: { ref: "pmd-nail01", name: "At-home nail care" },
          ...
          media: [
            { ref: "med-04a", url: null, alt: "Gel Removal Kit contents laid out" },
            { ref: "med-04b", url: null, alt: "Gel Removal Kit foil wrap on a nail" }
          ] },
        "prd-5e6a12": { ref: "prd-5e6a12", code: "rtl-beauty-05", name: "Hydration Serum", displayPrice: "$52.00", collection: { ref: "pmd-skin01", name: "Skin hydration ritual" },
          ...
          media: [
            { ref: "med-05a", url: null, alt: "Hydration Serum dropper bottle, front" },
            { ref: "med-05b", url: null, alt: "Hydration Serum dropper, close up" },
            { ref: "med-05c", url: null, alt: "Hydration Serum applied to the back of a hand" }
          ] },
        "prd-c130fb": { ref: "prd-c130fb", code: "rtl-beauty-06", name: "Overnight Mask", displayPrice: "$34.00", collection: null,
          ...
          media: [
            { ref: "med-06a", url: null, alt: "Overnight Mask jar, front" },
            { ref: "med-06b", url: null, alt: "Overnight Mask whipped texture, close up" }
          ] },
        "prd-longform": { ref: "prd-longform", code: "rtl-beauty-01", name: "Ultra-restorative overnight bond-recovery treatment mask with extended slow-release conditioning complex", displayPrice: "$64.00",
          ...
          variantFacts: [{ label: "Format", value: "Rinse-out treatment" }, { label: "Size", value: "200 ml" }],
          media: [
            { ref: "med-lfa", url: null, alt: "Long-name treatment jar, front label" },
            { ref: "med-lfb", url: null, alt: "Long-name treatment texture, close up" }
          ] }
      },
      /* ProductReview read model keyed by product ref. ONLY reviews whose backend
         workflow state is PUBLISHED are present here — moderation state, review /
         user / account / workflow ids and raw attributes are NEVER included.
         authorName is the backend-supplied public DISPLAY name (no generated
         person is presented as a real reviewer); verified is backend-supplied
         and renders "Verified purchase" only when true. publishedAt is optional.
         No aggregate score is supplied — the page shows the count of the visible
         loaded set only. */
      reviews: {
        "prd-7f3a91": [
        ...
        "prd-9d20c5": [
          { ref: "rev-8f20", productRef: "prd-9d20c5", rating: 5, title: "No more picking at gel", body: "Wrapped, waited, and the gel slid right off. My natural nails didn't peel for once.", authorName: "Lena K.", verified: true, publishedAt: "Jun 15, 2026" }
        ],
        "prd-5e6a12": [
  ...
  var spaBooking = {
    ...
    reviewLocation: "Harbor Front studio", /* WAVE 20: SUPERSEDED and no longer rendered — the review's visit mode and least-data place now come from spaBookingOptions (an assumed salon location was the defect this replaces). Kept only so a wave-16 diff stays readable; drop it on transfer. */
  ...
  /* ============================================================
     WAVE 20 — Booking OPTIONS read model (Calm Harbor target).
     CAPABILITY-DRIVEN: `capabilities` states what the server actually
     supports for the selected service / customer profile. A false
     capability means the section AND its step DO NOT EXIST — nothing is
     rendered as an empty choice. Everything else is server-returned
     display data: opaque mode codes / location refs / add-on refs,
     LEAST-DATA location labels (a nickname or a redacted summary, never
     a full address), display prices and duration notes rendered
     VERBATIM, and the server's notes maximum.
     Presentation NEVER infers compatibility, price, duration, tax,
     inventory or eligibility. An empty array means the source answered
     with no options; an absent / failing source is a SOURCE STATE
     (state.spaLocSrc / state.spaAddonSrc), never an empty choice.
     ============================================================ */
  var spaBookingOptions = {
    /* DEMO-SERVER ONLY — the amounts the demo quote sums. NEVER rendered:
       presentation only ever shows the server's display strings. */
    demoAmounts: {
    ...
    /* SERVER copy for every source state — rendered verbatim, never composed
       from a status code, permission failure or API name. */
    sourceCopy: {
    ...
    /* PREVIEW SEEDS (harness only) for the note counter / invalid states.
       A note is TRANSIENT browser state: never stored, logged or in a URL. */
    noteSamples: {
    ...
    /* one payload per reviewable scenario — the shape the server returns */
    scenarios: {
      /* 1. exactly one server-returned mode + place: FIXED CONTEXT, no choice,
            no Options step. The review shows what the source returned. */
      "fixed-studio": {
      ...
      /* 2. studio / at-home choice + redacted saved places */
      "mode-choice": {
      ...
      /* 3. eligible add-ons: long names, mixed / absent amounts, one REQUIRED
            choice the source marks and does not allow toggling */
      addons: {
      ...
      /* 4. all three capabilities populated (Options + Specialist rail) */
      all: {
  ...
  /* DEMO stand-in for the AUTHORITATIVE booking quote: the server returns the
     selected add-on lines and the display totals for the CURRENT selection.
     Presentation renders these strings verbatim and never adds an amount or a
     duration itself. displaySubtotal is returned only when the server has more
     than the treatment line to show. Codex replaces this with the real quote. */
  function spaBookingQuote(serviceCode, addonRefs, scenarioKey, priceOverrides) {
    ...
    var lines = [];
    (sc.addOns || []).forEach(function (a) {
      if ((addonRefs || []).indexOf(a.ref) === -1) return;
      var over = priceOverrides && priceOverrides[a.ref];
      sum += (over ? over.cents : (amt.addOns[a.ref] || 0));
      lines.push({ ref: a.ref, name: a.name, displayAmount: over ? over.displayPrice : a.displayPrice });
    });
    return {
      version: sc.selectionVersion,
      addOns: lines,
      /* the subtotal is returned only when the server has more than the
         treatment line to show \u2014 presentation never derives one */
