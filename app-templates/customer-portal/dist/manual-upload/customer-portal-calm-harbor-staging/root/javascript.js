(() => {
  ...
  function currentProduct() {
    ...
    if (state.config.dataMode !== "live") {
      ...
      if (fixtureDetail) return fixtureDetail;
      var fixtureProduct = productItems().find(function(item) {
        return opaqueRef("product", item.code) === state.spaCurrentProduct;
      });
  ...
  function opaqueRef(prefix, value) {
    var text7 = String(value || prefix);
    var left = 2166136261;
    var right = 2246822507;
    for (var index = 0; index < text7.length; index += 1) {
      var code = text7.charCodeAt(index);
  ...
  function addCoreCartItem(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight("cart:add:" + api.accountId + ":" + priceId + ":" + requestRef, async function() {
      ...
      var currencyId2 = positiveInteger(input && input.currencyId) || await currencyIdByCode(api, fetchImpl, text(input && input.currency) || before.currencyCode || defaultCurrency(api));
  ...
  function createPickupFulfillment(orderId2, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var id = positiveInteger2(orderId2);
    ...
    return (async function() {
      ...
      if (!positiveInteger2(Array.isArray(savedIds) && savedIds[0])) {
  ...
  async function pickupForOrder(api, fetchImpl, orderId2, marker) {
    ...
    return Object.assign({
      backendId: positiveInteger2(row.id),
  ...
  async function studioResource(api, fetchImpl) {
    ...
    var rows = Array.isArray(response && response.result) ? response.result : [];
    var row = rows[0] && positiveInteger2(rows[0].id) ? rows[0] : null;
  ...
  async function resolveOne(api, fetchImpl, url, filters, mappings, errorCode) {
    ...
    if (!row || !positiveInteger2(row.id)) throw contractError2(errorCode, errorCode.replace(/-/g, " "));
    if (mappings === TYPE_WITH_WORKFLOW_MAPPINGS && !(row.workflow && positiveInteger2(row.workflow.id))) {
  ...
  function writeContext(context, explicitOrigin) {
    ...
    var state2 = context && context.state || {};
    var session = context && context.session || state2.session || {};
    var accessToken = text2(session.accessToken || session.access_token);
    if (!accessToken) throw contractError2("session-required", "A Core access token is required");
    var organization = text2(config.organization);
    ...
    return {
      authorization: text2(session.tokenType || session.token_type || "Bearer") + " " + accessToken,
  ...
  async function loadCoreOrders(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var accountId = positiveInteger2(customer.id);
    ...
    var authorization = text2(session.tokenType || session.token_type || "Bearer") + " " + accessToken;
    var request = {
      filters: [{ type: "INTEGER", operator: "=", property: "account.id", value: String(accountId) }],
      mappings: ORDER_MAPPINGS,
      offset: 0,
      pageSize: positiveInteger2(config.ordersPageSize) || 50,
    ...
    var orderIds = rows.map(function(row) {
      return positiveInteger2(row && row.id);
    }).filter(Boolean);
  ...
  async function linesByOrder(context, orderIds) {
    ...
    var grouped = {};
    rows.forEach(function(row) {
      var id = positiveInteger2(row.order && row.order.id);
  ...
  function attributeNumber(row, code) {
    ...
    return entry && entry.value != null ? positiveInteger2(entry.value) : null;
  }
  function attributeText(row, code) {
    var entry = attributeEntry(row, code);
    return entry && entry.value != null ? text2(entry.value) : "";
  }
  function purchaseKind(lines) {
    var kinds = [];
    lines.forEach(function(line) {
      var kind = ITEM_KINDS[text2(line.type && line.type.code)];
  ...
  function lineTitle(line) {
    ...
    return localizedName(product && product.nls) || text2(product && product.code) || text2(line.notes) || "Item";
  }
  function normalizeLine2(line, currencyCode) {
    var id = positiveInteger2(line && line.id);
    var count = positiveInteger2(line.itemCount) || 1;
    var unit = finiteNumber2(line.amount);
    return {
      ref: "pln-core-" + id,
      kind: ITEM_KINDS[text2(line.type && line.type.code)] || null,
  ...
  function assertOwnedBy(row, accountId) {
    var rowAccountId = row && row.account && positiveInteger2(row.account.id);
  ...
  function normalizeOrder(row, accountId, rawLines, rawFulfillment) {
    assertOwnedBy(row, accountId);
    var id = positiveInteger2(row && row.id);
    ...
    var states = Array.isArray(row.states) ? row.states.map(function(state2) {
      return { id: positiveInteger2(state2 && state2.id), code: text2(state2 && state2.code), label: localizedName(state2 && state2.nls) };
    }) : [];
    ...
    var currencyCode = text2(row.currency && row.currency.code) || "USD";
    ...
    return {
      ...
      grandTotal: finiteNumber2(row.grandTotal),
      totalCharges: finiteNumber2(row.totalCharges),
      totalTaxes: finiteNumber2(row.totalTaxes),
      currency: { code: text2(row.currency && row.currency.code), label: localizedName(row.currency && row.currency.nls) },
      type: { code: text2(row.type && row.type.code), label: localizedName(row.type && row.type.nls) },
      states,
  ...
  function localizedName(value) {
    ...
    return text2(localized2 && (localized2.NAME || localized2.name));
  }
  ...
  function cancelCoreAppointment(ref, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2("appointment:cancel:" + id, async function() {
      ...
      if (!current || positiveInteger3(current.id) !== id) throw contractError3("appointment-not-found", "Core Appointment was not found");
      var accountId = positiveInteger3(api.customer && api.customer.id);
  ...
  async function loadCoreAppointments(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var response = await requestJson3(fetchImpl, api.serviceBase + "/api/appointment/list.json", requestOptions2(api, {
      ...
      offset: 0,
      pageSize: positiveInteger3(api.config.appointmentsPageSize) || 100,
      sorting: [{ field: "start", direction: "ASC" }]
    }));
    var rows = Array.isArray(response && response.result) ? response.result : [];
    var customerAccountId = positiveInteger3(api.customer && api.customer.id);
  ...
  function createCoreAppointment(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2(key, async function() {
      ...
      var durationMinutes = positiveInteger3(input && input.durationMinutes) || 60;
      var end = new Date(Date.parse(start) + durationMinutes * 6e4).toISOString();
      var serviceName = text3(input && input.serviceName) || "Spa appointment";
      ...
      var entity = {
        attributes: customerAttributes(api, visitType, {
          SERVICE_PRODUCT: positiveInteger3(input && input.serviceProductId) || null
        }),
        code,
        end,
      ...
      var id = positiveInteger3(Array.isArray(savedIds) && savedIds[0]);
  ...
  function createCoreOrder(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2("order:create:" + requestRef, async function() {
      var api = requestContext(context, explicitOrigin);
      var accountId = positiveInteger3(api.customer.id);
      ...
      var existing = rows.find(function(row) {
        return recordCodeOf(row) === marker || text3(row.notes) === marker;
      });
      ...
      var savedIds = await requestJson3(fetchImpl, api.billBase + "/api/order/save.json", requestOptions2(api, {
        entities: [{
          ...
          notes: text3(input && input.label) || "Customer portal checkout",
          organization: { id: organization.id },
  ...
  function plannedLine(line, index, itemTypes) {
    ...
    return { itemType, priceId, productCode: text3(line && line.productCode), qty, unitAmount };
  }
  async function itemTypesByCode(fetchImpl, api) {
    ...
    var byCode = {};
    rows.forEach(function(row) {
      if (row.workflow && positiveInteger3(row.workflow.id)) byCode[text3(row.code)] = row;
    });
    return byCode;
  }
  async function withOrderLines(fetchImpl, api, order, accountId) {
    ...
    return Object.assign({}, order, {
      ...
      lines: rows.map(function(row) {
        return {
          itemCount: finiteNumber3(row.itemCount, 0),
          priceId: positiveInteger3(row.itemPrice && row.itemPrice.id),
          productCode: text3(row.notes),
          typeCode: text3(row.type && row.type.code),
          unitAmount: finiteNumber3(row.amount, null)
        };
      })
    });
  }
  ...
  function resolveTypeWithWorkflow(fetchImpl, api, base, endpoint, code, errorCode) {
    return resolveByCode(fetchImpl, api, base, endpoint, code, TYPE_WITH_WORKFLOW_MAPPINGS2, errorCode).then(function(row) {
      if (!row.workflow || !positiveInteger3(row.workflow.id))
  ...
  async function ensureVisitTask(fetchImpl, api, options2) {
    ...
    if (existing) return positiveInteger3(existing.id);
  ...
  function customerAttributeId(row, code) {
    ...
    return entry && entry.value != null ? positiveInteger3(entry.value) : null;
  }
  ...
  function requestContext(context, explicitOrigin) {
    ...
    return {
      ...
      customer,
      customerUserId: positiveInteger3(session.userId || state2.session && state2.session.userId) || null,
      organization,
  ...
  function normalizeAppointment(row) {
    ...
    var states = Array.isArray(row.states) ? row.states.map(function(state2) {
      return text3(state2 && state2.code);
    }).filter(Boolean) : [];
    ...
    return {
      ...
      optimistic: Number.isFinite(Number(row.optimistic)) ? Number(row.optimistic) : null,
      code: text3(row.code),
      service: localizedName2(row.nls) || text3(row.code) || "Spa appointment",
      startIso: start,
      endIso: end,
  ...
  function normalizeOrder2(row, accountId) {
    var rowAccountId = row && row.account && positiveInteger3(row.account.id);
    ...
    return {
      ...
      grandTotal: finiteNumber3(row.grandTotal, 0),
      totalCharges: finiteNumber3(row.totalCharges, 0),
      totalTaxes: finiteNumber3(row.totalTaxes, 0),
      currencyCode: text3(row.currency && row.currency.code),
      typeCode: text3(row.type && row.type.code),
  ...
  function appointmentId(ref) {
    ...
    var id = positiveInteger3(value.slice(REF_PREFIX.length));
  ...
  function requiredRef(value, label) {
    var id = value && positiveInteger3(value.id);
  ...
  function idempotencyPart(value) {
    var clean = text3(value).toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "");
  ...
  async function loadAccountPhone(api, fetchImpl) {
    ...
    if (!account || positiveInteger4(account.id) !== api.accountId) return null;
    ...
    for (var index = 0; index < contacts.length; index += 1) {
      ...
      for (var entryIndex = 0; entryIndex < entries.length; entryIndex += 1) {
        ...
        if (text4(entry.type && entry.type.code) === "PHONE" && text4(entry.value)) return text4(entry.value);
      }
    }
    return null;
  }
  async function saveCoreUserProfile(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var entity = {
      email: text4(input && input.email),
      enabled: current.enabled !== false,
      fullname: text4(current.fullname),
      id: api.userId,
      language: requiredRef2(current.language, "User language"),
      name: text4(current.name),
  ...
  function normalize(row, userId, phone) {
    if (!row || positiveInteger4(row.id) !== userId) throw error("profile-scope-mismatch", "Core User readback did not match the signed-in User");
    return {
      state: "ready",
      email: text4(row.email),
      phone: phone || null,
      prefs: {},
      optimistic: Number.isFinite(Number(row.optimistic)) ? Number(row.optimistic) : null,
  ...
  function requestContext2(context, explicitOrigin) {
    ...
    var state2 = context && context.state || {};
    var session = context && context.session || state2.session || {};
    var token = text4(session.accessToken || session.access_token);
    var userId = positiveInteger4(session.userId);
    ...
    return {
      ...
      accountId: positiveInteger4(customer.id) || null,
      base,
      token: text4(session.tokenType || session.token_type || "Bearer") + " " + token,
      userId
    };
  }
  ...
  function requiredRef2(value, label) {
    var id = value && positiveInteger4(value.id);
  ...
  function normalizeAuthored(payload, options2) {
    ...
    var trust = array(own(content, "trust", "content"), "content.trust", 0).map(function(fact2, index) {
  ...
  function oidcStep(n, text7) {
    return h("div", { "class": "oidc-step" }, [h("span", { "class": "oidc-step__num" }, String(n)), text7]);
  }
  ...
  function pushChat(text7) {
    if (!text7 || !text7.trim()) throw new Error("Message is empty");
    state.messages = state.messages.concat([{ from: "user", text: text7.trim() }]);
    state.chatInput = "";
      ...
      return {
        pimProducts: plans.map(function(product) {
          ...
          return Object.assign({}, product, {
            ref: opaqueRef2("product", product.code),
  ...
  function normalizeModels(rows, productCodes) {
    return rows.map(function(row) {
      ...
      return {
        ref: opaqueRef2("product-model", row.code),
  ...
  function normalizeReviews(rows, productCodes) {
    return rows.map(function(row) {
      ...
      return {
        ref: opaqueRef2("product-review", identity),
        productCode,
        rating,
  ...
  async function resolveCoreAccount(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var tokenType = text5(session.tokenType || session.token_type || "Bearer");
    ...
    var userId = positiveInteger5(basicInfo.authenticatedUserId || basicInfo.id);
    ...
    var accountTypeCode = text5(config.accountTypeCode || "SPA_CUSTOMER");
    ...
    if (!positiveInteger5(account.id)) throw contractError5("invalid-customer-account", "Core Account response did not include an id");
    if (account.user && positiveInteger5(account.user.id) && Number(account.user.id) !== userId) {
    ...
    return {
      ...
      user: {
        id: userId,
        displayName: text5(basicInfo.authenticatedUserName || basicInfo.authenticatedUser || basicInfo.name)
      },
      account: {
        id: Number(account.id),
        code: text5(account.code),
        displayName: localizedName3(account.nls) || text5(account.code),
  ...
  function selectOrganization(configuredCode, basicInfo) {
    var configured = text5(configuredCode);
    var authorized = Array.isArray(basicInfo && basicInfo.authorizedOrganizations) ? basicInfo.authorizedOrganizations.map(function(item) {
      return text5(item && item.code);
    }).filter(Boolean) : [];
    var current = text5(basicInfo && (basicInfo.organizationCode || basicInfo.defaultOrganizationCode));
  ...
  async function loadEnrollments(api, accountId, fetchImpl) {
    var response = await requestJson6(fetchImpl, api.serviceBase + "/api/project/list.json", requestOptions3(api, {
      ...
      offset: 0,
      pageSize: positiveInteger6(api.config.plansPageSize) || 100
    }));
    var rows = Array.isArray(response && response.result) ? response.result : [];
  ...
  function normalizeEnrollment(row) {
    ...
    return {
      ...
      kind: "PACKAGE",
      optimistic: finiteNumber5(row.optimistic, null),
      ref: PLAN_REF_PREFIX + id,
      remainingUses: remaining,
      sourcePurchase: sourceOrder ? PURCHASE_REF_PREFIX + sourceOrder : null,
      status,
      title: localizedName4(row.nls) || text6(row.code) || "Package",
      totalUses: total
    };
  }
  ...
  function currencyId(line) {
    ...
    return entry ? positiveInteger6(entry.value) : 0;
  }
  function intervalUnitId(line) {
    var entry = priceAttributeEntry(line, "INTERVAL");
    return entry ? positiveInteger6(entry.unit) : 0;
  }
  async function codesById(api, base, endpoint, ids, fetchImpl) {
    ...
    (Array.isArray(response && response.result) ? response.result : []).forEach(function(row) {
      byId[String(row.id)] = text6(row.code);
    });
    return byId;
  }
  ...
  async function membershipLines(api, subscriptions, fetchImpl) {
    ...
    rows.forEach(function(row) {
      ...
      if (orderIds.indexOf(positiveInteger6(row.order && row.order.id)) >= 0 && !byOrder[key]) byOrder[key] = row;
    });
    return byOrder;
  }
  ...
  function firstState(row) {
    ...
    for (var index = 0; index < states.length; index += 1) {
      var code = text6(states[index] && states[index].code);
      if (code) return code;
    }
    return "";
  }
  function orderId(row) {
    return positiveInteger6(row && row.order && row.order.id);
  }
  function membershipTitle(line) {
    if (!line) return "Membership";
    var product = line.itemPrice && line.itemPrice.product;
    return localizedName4(product && product.nls) || localizedName4(line.itemPrice && line.itemPrice.nls) || "Membership";
  }
  ...
  function localizedName4(nls) {
    ...
    for (var index = 0; index < locales.length; index += 1) {
      ...
      var name = entry && text6(entry.NAME);
      if (name) return name;
    }
    return "";
  }
  function sameOriginBase6(base, origin, label) {
    var value = text6(base);
  ...
  function stepsBar(f) {
    ...
    var steps = f.entry === "reschedule" ? [{ k: "slots", l: "New time" }, { k: "review", l: "Review" }] : [{ k: "context", l: "Service" }].concat(hasSpec ? [{ k: "specialist", l: "Specialist" }] : []).concat([{ k: "slots", l: "Time" }, { k: "review", l: "Review" }]);
