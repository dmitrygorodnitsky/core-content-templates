(() => {
    ...
    overview: {
      ...
      properties: [
        ...
        },
        {
          ...
          address: "870 Cinnamon Bear Way, Lakewood, CO 80227",
          lat: 39.67564,
          lon: -105.08018,
          zone: "south",
          contract: "1241",
          quoteSiteId: "gr-cinnamon",
          appointment: null,
          ticket: null,
  ...
  function addCoreCartItem(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    var api = cartContext(context, explicitOrigin);
    var priceId = positiveInteger3(input && input.priceId);
    if (!priceId) throw contractError("cart-price-required", "A Core price id is required to add a cart item");
    var productId = positiveInteger3(input && input.productId);
    if (!productId) throw contractError("cart-product-required", "A Core product id is required to add a cart item");
    var count = positiveInteger3(input && input.count) || 1;
    var requestRef = text3(input && input.requestRef) || String(priceId) + "x" + count;
    return singleFlight("cart:add:" + api.accountId + ":" + priceId + ":" + requestRef, async function() {
      var before = await readCart(api, fetchImpl);
      var currencyId2 = positiveInteger3(input && input.currencyId) || await currencyIdByCode(api, fetchImpl, text3(input && input.currency) || before.currencyCode || defaultCurrency(api));
      await requestJson(fetchImpl, api.billBase + ITEMS_PATH, requestOptions(api, "POST", {
        ...
        notes: text3(input && input.notes) || null,
        priceId,
        productId
      }));
  ...
  function normalizeCart(view, api, codes) {
    ...
    var currencyCode = currencyOf(view && view.currency) || (lineCodes.length === 1 ? lineCodes[0] : "") || defaultCurrency(api);
    var lines = rows.map(function(row) {
      return normalizeLine(row, currencyCode, api, codes);
    });
    var byRef = {};
  ...
  function normalizeLine(row, cartCurrency, api, codes) {
    ...
    return {
      backendId: positiveInteger3(row && row.id) || null,
      currencyCode,
  ...
  function catalogEntry(api, productCode, productId) {
    ...
    for (var index = 0; index < items.length; index += 1) {
      ...
      var code = text3(item && (item.code || item.sku));
      if (productCode && code && code === productCode) return item;
      if (productId && positiveInteger3(item && (item.backendProductId || item.backendId || item.id)) === productId) return item;
    }
    return null;
  }
  function currencyIdsIn(view) {
    ...
    rows.concat([view || {}]).forEach(function(row) {
      var id = positiveInteger3(row && row.currency);
  ...
  async function currencyCodesByIds(api, fetchImpl, ids) {
    ...
    results.forEach(function(rows) {
      rows.forEach(function(row) {
        byId[String(row.id)] = text3(row.code);
      });
    });
    return byId;
  }
  async function currencyIdByCode(api, fetchImpl, code) {
    ...
    var id = positiveInteger3(rows[0] && rows[0].id);
  ...
  function cartPriceId(input) {
    var direct = positiveInteger3(input && input.priceId);
    if (direct) return direct;
    var ref = text3(input && (input.ref || input));
    if (ref.indexOf(LINE_REF_PREFIX) === 0) {
      var parsed = positiveInteger3(ref.slice(LINE_REF_PREFIX.length));
  ...
  function cartContext(context, explicitOrigin) {
    ...
    var accountId = positiveInteger3(customer && customer.id);
  ...
  function defaultCurrency(api) {
    return text3(api.config.currency || api.config.pimCurrency) || "USD";
  }
  ...
  async function requestJson(fetchImpl, url, options2) {
    ...
    if (!response.ok) {
      ...
      var error2 = contractError(code, "Core cart request failed with HTTP " + response.status);
  ...
  function createCoreOrdersAdapter(options2 = {}) {
    var fetchImpl = options2.fetch || globalThis.fetch;
    if (typeof fetchImpl !== "function") throw contractError2("fetch-unavailable", "Core Orders adapter requires fetch");
    return {
      async load(moduleId, context) {
        if (moduleId !== "orders") throw contractError2("unsupported-module", "Core Orders adapter cannot load " + moduleId);
  ...
  function createPickupFulfillment(orderId2, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var id = positiveInteger4(orderId2);
    if (!id) throw contractError2("fulfillment-order-required", "A Core Order id is required to record a pickup");
    ...
    return (async function() {
      ...
      var savedIds = await requestJson2(fetchImpl, api.billBase + "/api/shipment/save.json", writeOptions(api, {
        entities: [{
          attributes,
      ...
      if (!positiveInteger4(Array.isArray(savedIds) && savedIds[0])) {
        throw contractError2("invalid-save-response", "Core Shipment save did not return an id");
      }
      var readback = await pickupForOrder(api, fetchImpl, id, marker);
      if (!readback) throw contractError2("fulfillment-unconfirmed", "Core did not report the pickup record after the save");
  ...
  async function pickupForOrder(api, fetchImpl, orderId2, marker) {
    var response = await requestJson2(fetchImpl, api.billBase + "/api/shipment/list.json", writeOptions(api, {
    ...
    var row = rows.find(function(candidate) {
      return attributeNumber(candidate, "SOURCE_ORDER") === orderId2 && attributeText(candidate, "RECORD_CODE") === marker;
    });
    if (!row) return null;
    ...
    return Object.assign({
      backendId: positiveInteger4(row.id),
  ...
  async function studioResource(api, fetchImpl) {
    var response;
    try {
      response = await requestJson2(fetchImpl, api.rmBase + "/api/resource/list.json", writeOptions(api, {
    ...
    var rows = Array.isArray(response && response.result) ? response.result : [];
    var row = rows[0] && positiveInteger4(rows[0].id) ? rows[0] : null;
  ...
  async function resolveOne(api, fetchImpl, url, filters, mappings, errorCode) {
    var response = await requestJson2(fetchImpl, url, writeOptions(api, { filters, mappings, offset: 0, pageSize: 1 }));
    var row = (Array.isArray(response && response.result) ? response.result : [])[0];
    if (!row || !positiveInteger4(row.id)) throw contractError2(errorCode, errorCode.replace(/-/g, " "));
    if (mappings === TYPE_WITH_WORKFLOW_MAPPINGS && !(row.workflow && positiveInteger4(row.workflow.id))) {
      throw contractError2(errorCode, FULFILLMENT_TYPE + " has no workflow");
    }
    return row;
  }
  function writeContext(context, explicitOrigin) {
    ...
    return {
      ...
      organization,
      rmBase: sameOriginBase2(config.resourceApiBase || "/core-rm", origin, "Core Resource API base")
    };
  }
  ...
  async function loadCoreOrders(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    if (!accountId) throw contractError2("customer-account-required", "Resolved customer Account is required before Orders load");
    var accessToken = text4(session.accessToken || session.access_token);
    if (!accessToken) throw contractError2("session-required", "A Core access token is required");
    var organization = text4(config.organization);
    if (!organization) throw contractError2("organization-required", "Verified portal organization is required");
    var origin = explicitOrigin || config.origin || browserOrigin2();
    var billBase = sameOriginBase2(config.billApiBase || "/core-bill", origin, "Core Bill API base");
    ...
    var response = await requestJson2(fetchImpl, billBase + "/api/order/list.json", {
      method: "POST",
      credentials: "same-origin",
      headers: {
    ...
    var orderIds = rows.map(function(row) {
      return positiveInteger4(row && row.id);
    }).filter(Boolean);
  ...
  async function postList(context, path, body) {
    return requestJson2(context.fetchImpl, context.billBase + path, {
      method: "POST",
      credentials: "same-origin",
  ...
  async function linesByOrder(context, orderIds) {
    ...
    var grouped = {};
    rows.forEach(function(row) {
      var id = positiveInteger4(row.order && row.order.id);
  ...
  function attributeNumber(row, code) {
    ...
    return entry && entry.value != null ? positiveInteger4(entry.value) : null;
  }
  function attributeText(row, code) {
    var entry = attributeEntry(row, code);
    return entry && entry.value != null ? text4(entry.value) : "";
  }
  function purchaseKind(lines) {
    var kinds = [];
    lines.forEach(function(line) {
      var kind = ITEM_KINDS[text4(line.type && line.type.code)];
  ...
  function lineTitle(line) {
    ...
    return localizedName2(product && product.nls) || text4(line.notes) || "Item";
  }
  function normalizeLine2(line, currencyCode) {
    ...
    return {
      ...
      variant: null,
      quantity: count,
      displayUnitPrice: unit != null ? formatMoney2(unit, currencyCode) : null,
      displayTotal: null
    };
  }
  ...
  function assertOwnedBy(row, accountId) {
    ...
    if (rowAccountId !== accountId) throw contractError2("order-scope-mismatch", "Core Order does not belong to the resolved customer Account");
  }
  function normalizeOrder(row, accountId, rawLines, rawFulfillment) {
    assertOwnedBy(row, accountId);
    var id = positiveInteger4(row && row.id);
    if (!id) throw contractError2("invalid-order", "Core Order response did not include an id");
    var states = Array.isArray(row.states) ? row.states.map(function(state2) {
      return { id: positiveInteger4(state2 && state2.id), code: text4(state2 && state2.code), label: localizedName2(state2 && state2.nls) };
    }) : [];
    ...
    return {
      ...
      displayTotal: formatMoney2(row.grandTotal, currencyCode),
      displayCurrency: currencyCode,
      attention,
      allowedActions: [],
      lines,
      money: {
        subtotal: formatMoney2(row.totalCharges, currencyCode),
        tax: formatMoney2(row.totalTaxes, currencyCode),
        total: formatMoney2(row.grandTotal, currencyCode),
  ...
  async function requestJson2(fetchImpl, url, options2) {
    var response = await fetchImpl(url, options2);
    if (!response || typeof response.ok !== "boolean") throw contractError2("invalid-response", "Core Bill returned an invalid response");
    if (!response.ok) {
      var code = response.status === 401 ? "session-expired" : response.status === 403 ? "orders-forbidden" : "orders-request-failed";
      var error2 = contractError2(code, "Core Orders request failed with HTTP " + response.status);
      error2.status = response.status;
      throw error2;
    }
    try {
      ...
      throw contractError2("invalid-response", "Core Orders response was not valid JSON");
    }
  }
  function sameOriginBase2(value, origin, label) {
    if (!origin) throw contractError2("origin-required", label + " requires a browser origin");
    var target = new URL(String(value || ""), origin);
    if (target.origin !== new URL(origin).origin) throw contractError2("cross-origin-service", label + " must be same-origin");
  ...
  var ORDER_MAPPINGS2 = [
    ...
    { name: "totalCharges" },
    { name: "totalTaxes" },
    { key: "id", mappings: REF_MAPPINGS2, name: "account", type: "identifier" },
    { key: "id", mappings: REF_MAPPINGS2, name: "currency", type: "identifier" },
    { key: "id", name: "organization", type: "identifier" },
    { mappings: REF_MAPPINGS2, name: "states", type: "collection" },
    { key: "id", mappings: REF_MAPPINGS2, name: "type", type: "identifier" },
    { key: "id", mappings: REF_MAPPINGS2, name: "workflow", type: "identifier" }
  ...
  function createCoreSpaDemoAdapter(options2 = {}) {
    var fetchImpl = options2.fetch || globalThis.fetch;
    if (typeof fetchImpl !== "function") throw contractError3("fetch-unavailable", "Core SPA demo adapter requires fetch");
    return {
      async load(moduleId, context) {
        if (moduleId !== "appointments") throw contractError3("unsupported-module", "Core SPA demo adapter cannot load " + moduleId);
  ...
  function cancelCoreAppointment(ref, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2("appointment:cancel:" + id, async function() {
      ...
      if (!current || positiveInteger5(current.id) !== id) throw contractError3("appointment-not-found", "Core Appointment was not found");
      var accountId = positiveInteger5(api.customer && api.customer.id);
      if (!accountId || customerAttributeId(current, "CUSTOMER_ACCOUNT") !== accountId) {
        throw contractError3("appointment-forbidden", "This appointment does not belong to the signed-in customer");
      }
      ...
      if (before.allowedActions.indexOf("cancel") === -1) {
        throw contractError3("appointment-not-cancellable", "This appointment cannot be cancelled in its current state");
      }
      ...
      if (readback.customerStatus !== "Cancelled") {
        throw contractError3("appointment-cancel-unconfirmed", "Core did not report the appointment as cancelled");
      }
      return readback;
    });
  }
  async function loadCoreAppointments(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    var api = requestContext(context, explicitOrigin);
    var response = await requestJson3(fetchImpl, api.serviceBase + "/api/appointment/list.json", requestOptions2(api, {
      filters: [{ type: "STRING", operator: "=", property: "type.code", value: APPOINTMENT_TYPE }],
      mappings: APPOINTMENT_MAPPINGS,
      offset: 0,
      pageSize: positiveInteger5(api.config.appointmentsPageSize) || 100,
      sorting: [{ field: "start", direction: "ASC" }]
    }));
    var rows = Array.isArray(response && response.result) ? response.result : [];
    var customerAccountId = positiveInteger5(api.customer && api.customer.id);
    ...
    try {
      var resourceResults = await Promise.all([RESOURCE_TYPE, LOCATION_TYPE].map(function(typeCode) {
        return requestJson3(fetchImpl, api.resourceBase + "/api/resource/list.json", requestOptions2(api, {
          filters: [{ type: "STRING", operator: "=", property: "type.code", value: typeCode }],
          mappings: RESOURCE_MAPPINGS,
          offset: 0,
          pageSize: positiveInteger5(api.config.resourcesPageSize) || 100,
  ...
  function createCoreAppointment(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2(key, async function() {
      var api = requestContext(context, explicitOrigin);
      var accountId = positiveInteger5(api.customer && api.customer.id);
      if (!accountId) throw contractError3("customer-account-required", "Resolved customer Account is required before booking");
      ...
      var durationMinutes = positiveInteger5(input && input.durationMinutes) || 60;
      var end = new Date(Date.parse(start) + durationMinutes * 6e4).toISOString();
      var serviceName = text5(input && input.serviceName) || "Spa appointment";
      var serviceProductId = positiveInteger5(input && input.serviceProductId);
      if (!serviceProductId) throw contractError3("service-product-required", "A Core Product is required before booking");
      var resourceIds = Array.from(new Set((Array.isArray(input && input.resourceIds) ? input.resourceIds : []).map(positiveInteger5).filter(Boolean)));
      if (!resourceIds.length) throw contractError3("appointment-resources-required", "A bookable Resource is required before booking");
      var specialistAccountId = positiveInteger5(input && input.specialistAccountId);
      if (!specialistAccountId) throw contractError3("specialist-account-required", "A specialist Account is required before booking");
      ...
      var entity = {
        attributes: customerAttributes(api, visitType, {
          ...
          BOOKING_OPTIONS_VERSION: text5(input && input.bookingOptionsVersion) || null,
          CUSTOMER_NOTE: text5(input && input.customerNote) || null,
          LOCATION_LABEL: text5(input && input.locationLabel) || null,
          LOCATION_RESOURCE: positiveInteger5(input && input.locationResourceId),
          REQUEST_REF: requestRef,
          RESOURCES: resourceIds,
          SERVICE_PRODUCT: serviceProductId,
          SPECIALIST_ACCOUNT: specialistAccountId,
          VISIT_MODE: text5(input && input.visitMode) || null
        }),
        code,
        end,
      ...
      var savedIds = await requestJson3(fetchImpl, api.serviceBase + "/api/appointment/save.json", requestOptions2(api, {
        entities: [entity],
        mappings: APPOINTMENT_MAPPINGS
      }));
      var id = positiveInteger5(Array.isArray(savedIds) && savedIds[0]);
      if (!id) throw contractError3("invalid-save-response", "Core Appointment save did not return an id");
  ...
  function customerFlightPart(context) {
    ...
    var session = context && context.session || state2.session || {};
    var customer = context && context.account || state2.customerAccount || session.account || {};
    return positiveInteger5(customer.id) || "unresolved";
  }
  async function confirmRequestedAppointment(row, api, fetchImpl, accountId, input) {
    if (!row || !positiveInteger5(row.id)) throw contractError3("appointment-not-found", "Core Appointment was not found after booking");
    if (customerAttributeId(row, "CUSTOMER_ACCOUNT") !== accountId) {
      throw contractError3("appointment-forbidden", "Booking idempotency resolved another customer's appointment");
    }
    ...
    if (before.customerStatus !== "Requested") {
      throw contractError3("appointment-request-unavailable", "Core did not keep the booking in a confirmable request state");
    }
    try {
      await requestCommand(
        fetchImpl,
    ...
    if (confirmed.customerStatus !== "Confirmed") {
      throw contractError3("appointment-confirm-unconfirmed", "Core did not report the appointment as scheduled");
    }
    assertNormalizedBookingSelection(confirmed, input);
  ...
  function rescheduleCoreAppointment(ref, input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2("appointment:reschedule:" + id, async function() {
      ...
      var duration = (positiveInteger5(input && input.durationMinutes) || Math.round(currentDuration / 6e4)) * 6e4;
      var entity = {
        attributes: mergeAppointmentAttributes(current.attributes, current.type, {
          ...
          LOCATION_LABEL: text5(input && input.locationLabel) || null,
          LOCATION_RESOURCE: positiveInteger5(input && input.locationResourceId),
          RESOURCES: Array.from(new Set((Array.isArray(input && input.resourceIds) ? input.resourceIds : []).map(positiveInteger5).filter(Boolean))),
          SERVICE_PRODUCT: positiveInteger5(input && input.serviceProductId),
          SPECIALIST_ACCOUNT: positiveInteger5(input && input.specialistAccountId),
          VISIT_MODE: text5(input && input.visitMode) || null
        }),
        code: text5(current.code),
      ...
      await requestJson3(fetchImpl, api.serviceBase + "/api/appointment/save.json", requestOptions2(api, {
        entities: [entity],
        mappings: APPOINTMENT_MAPPINGS
      }));
  ...
  function createCoreOrder(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    return singleFlight2("order:create:" + requestRef, async function() {
      var api = requestContext(context, explicitOrigin);
      var accountId = positiveInteger5(api.customer.id);
      if (!accountId) throw contractError3("customer-account-required", "Resolved customer Account is required before checkout");
      if (!lines.length) throw contractError3("order-lines-required", "An order needs at least one cart line");
      ...
      ], ORDER_MAPPINGS2, [{ field: "id", direction: "DESC" }]);
      var existing = rows.find(function(row) {
        return recordCodeOf(row) === marker || text5(row.notes) === marker;
      });
      ...
      var savedIds = await requestJson3(fetchImpl, api.billBase + "/api/order/save.json", requestOptions2(api, {
        entities: [{
          account: { id: accountId },
          attributes: recordCodeAttributes(orderType, marker),
          currency: { id: currency.id },
          // Totals are NOT sent — Core computes grandTotal from the lines.
          notes: text5(input && input.label) || "Customer portal checkout",
          organization: { id: organization.id },
      ...
      var id = positiveInteger5(Array.isArray(savedIds) && savedIds[0]);
      if (!id) throw contractError3("invalid-save-response", "Core Order save did not return an id");
      for (var index = 0; index < planned.length; index += 1) {
        var line = planned[index];
        await requestJson3(fetchImpl, api.billBase + "/api/order-item/save.json", requestOptions2(api, {
          entities: [{
      ...
      var readback = await getEntity(fetchImpl, api.billBase + "/api/order/get.json?id=" + id, api, ORDER_MAPPINGS2);
  ...
  function plannedLine(line, index, itemTypes) {
    var priceId = positiveInteger5(line && line.priceId);
    ...
    if (!itemType) throw contractError3("order-item-type-missing", itemTypeCode + " is not provisioned in this organization");
    return { itemType, priceId, productCode: text5(line && line.productCode), qty, unitAmount };
  }
  async function itemTypesByCode(fetchImpl, api) {
    ...
    var byCode = {};
    rows.forEach(function(row) {
      if (row.workflow && positiveInteger5(row.workflow.id)) byCode[text5(row.code)] = row;
    });
    return byCode;
  }
  ...
  async function resolveByCode(fetchImpl, api, base, endpoint, code, mappings, errorCode) {
    ...
    if (!row) throw contractError3(errorCode || "reference-missing", endpoint + " " + code + " is not provisioned in this organization");
    return row;
  }
  function resolveTypeWithWorkflow(fetchImpl, api, base, endpoint, code, errorCode) {
    return resolveByCode(fetchImpl, api, base, endpoint, code, TYPE_WITH_WORKFLOW_MAPPINGS2, errorCode).then(function(row) {
      if (!row.workflow || !positiveInteger5(row.workflow.id))
        throw contractError3(errorCode || "reference-missing", endpoint + " " + code + " has no workflow");
      return row;
    });
  }
  ...
  function orderCurrencyCode(api) {
    return text5(api.config.currency || api.config.pimCurrency) || "USD";
  }
  var TASK_MAPPINGS = [
    { name: "attributes" },
    { name: "code" },
  ...
  async function resolveCarePlan(fetchImpl, api) {
    ...
    if (!owned) throw contractError3("care-plan-missing", "This customer has no care plan to record a visit against");
    return owned;
  }
  async function ensureVisitTask(fetchImpl, api, options2) {
    ...
    if (existing) return positiveInteger5(existing.id);
    ...
    var reusable = careTasks.find(function(row) {
      return positiveInteger5(row && row.project && row.project.id) === positiveInteger5(options2.carePlan && options2.carePlan.id);
    });
    if (reusable && positiveInteger5(reusable.id)) return positiveInteger5(reusable.id);
    var savedIds = await requestJson3(fetchImpl, api.serviceBase + "/api/task/save.json", requestOptions2(api, {
      entities: [{
    ...
    var id = positiveInteger5(Array.isArray(savedIds) && savedIds[0]);
    if (!id) throw contractError3("invalid-save-response", "Core Task save did not return an id");
    return id;
  }
  async function listOne(fetchImpl, url, api, filters, mappings, sorting) {
    ...
    var response = await requestJson3(fetchImpl, url, requestOptions2(api, body));
    var rows = Array.isArray(response && response.result) ? response.result : [];
    return rows[0] || null;
  }
  ...
  function assertBookingSelection(row, input) {
    ...
    var expected = {
      ...
      bookingOptionsVersion: text5(input.bookingOptionsVersion),
      customerNote: text5(input.customerNote),
      locationLabel: text5(input.locationLabel),
      locationResourceId: positiveInteger5(input.locationResourceId),
      visitMode: text5(input.visitMode)
    };
    var observed = {
      addOnRefs: textList(appointmentAttributeValue(row, "ADD_ON_REFS")),
      bookingOptionsVersion: text5(appointmentAttributeValue(row, "BOOKING_OPTIONS_VERSION")),
      customerNote: text5(appointmentAttributeValue(row, "CUSTOMER_NOTE")),
      locationLabel: text5(appointmentAttributeValue(row, "LOCATION_LABEL")),
      locationResourceId: positiveInteger5(appointmentAttributeValue(row, "LOCATION_RESOURCE")),
      visitMode: text5(appointmentAttributeValue(row, "VISIT_MODE"))
    };
    if (JSON.stringify(expected) !== JSON.stringify(observed)) {
      throw contractError3("booking-selection-readback-mismatch", "Core Appointment did not retain the selected booking options");
    }
  }
  function assertNormalizedBookingSelection(appointment, input) {
    if (!input) return;
    var expectedRefs = textList(input.addOnRefs);
    if (appointment.visitMode !== text5(input.visitMode) || appointment.location !== text5(input.locationLabel) || appointment.bookingOptionsVersion !== text5(input.bookingOptionsVersion) || appointment.customerNote !== text5(input.customerNote) || JSON.stringify(appointment.addOnRefs) !== JSON.stringify(expectedRefs)) {
      throw contractError3("booking-selection-readback-mismatch", "Confirmed Appointment did not retain the selected booking options");
    }
  }
  function getEntity(fetchImpl, url, api, mappings) {
    return requestJson3(fetchImpl, url, requestOptions2(api, mappings));
  }
  function requestContext(context, explicitOrigin) {
    ...
    return {
      ...
      customer,
      customerUserId: positiveInteger5(session.userId || state2.session && state2.session.userId) || null,
      organization,
      resourceBase: sameOriginBase3(config.resourceApiBase || "/core-rm", origin, "Core Resource API base"),
      serviceBase: sameOriginBase3(config.serviceApiBase || "/core-svc", origin, "Core Service API base")
    };
  }
  ...
  async function requestResponse(fetchImpl, url, options2) {
    ...
    if (!response.ok) {
      ...
      var error2 = contractError3(code, "Core request failed with HTTP " + response.status + (detail ? " \u2014 " + detail : ""));
      error2.status = response.status;
  ...
  async function failureDetail(response) {
    try {
      ...
      raw = text5(raw).replace(/\s+/g, " ").slice(0, 500);
      if (!raw || raw === "{}") return "";
      try {
        var parsed = JSON.parse(raw);
        return text5(parsed && (parsed.message || parsed.error || parsed.detail)) || raw;
      } catch (_) {
        return raw;
      }
    } catch (_) {
  ...
  function normalizeAppointment(row) {
    ...
    if (!id) throw contractError3("invalid-appointment", "Core Appointment response did not include an id");
    var start = validIso(row.start);
    var end = validIso(row.end);
    var states = Array.isArray(row.states) ? row.states.map(function(state2) {
      return text5(state2 && state2.code);
    }).filter(Boolean) : [];
    ...
    return {
      ...
      optimistic: Number.isFinite(Number(row.optimistic)) ? Number(row.optimistic) : null,
      code: text5(row.code),
      service: localizedName3(row.nls) || "Spa appointment",
      startIso: start,
      endIso: end,
      ...
      specialist: "",
      mode: text5(appointmentAttributeValue(row, "VISIT_MODE")) || "salon",
      visitMode: text5(appointmentAttributeValue(row, "VISIT_MODE")) || "salon",
      location: text5(appointmentAttributeValue(row, "LOCATION_LABEL")),
      locationResourceId: positiveInteger5(appointmentAttributeValue(row, "LOCATION_RESOURCE")),
      addOnRefs: textList(appointmentAttributeValue(row, "ADD_ON_REFS")),
      customerNote: text5(appointmentAttributeValue(row, "CUSTOMER_NOTE")),
      bookingOptionsVersion: text5(appointmentAttributeValue(row, "BOOKING_OPTIONS_VERSION")),
  ...
  function appointmentId(ref) {
    var value = text5(ref);
    if (!value.startsWith(REF_PREFIX)) throw contractError3("invalid-appointment-ref", "Appointment reference is invalid");
    var id = positiveInteger5(value.slice(REF_PREFIX.length));
    if (!id) throw contractError3("invalid-appointment-ref", "Appointment reference is invalid");
    return id;
  }
  ...
  function textList(value) {
    return (Array.isArray(value) ? value : value == null || value === "" ? [] : [value]).map(text5).filter(Boolean).sort();
  }
  ...
  function requiredRef(value, label) {
    var id = value && positiveInteger5(value.id);
    if (!id) throw contractError3("template-reference-missing", label + " is missing");
    return { id };
  }
  ...
  function validIso(value) {
    var date = new Date(value);
    if (!Number.isFinite(date.getTime())) throw contractError3("invalid-date", "Appointment date is invalid");
    return date.toISOString();
  }
  function idempotencyPart(value, maxLength) {
    var clean = text5(value).toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "") || "REQUEST";
    var limit = positiveInteger5(maxLength) || 32;
  ...
  async function loadCoreOidcSession(config) {
    if (!globalThis.oidc || !globalThis.oidc.UserManager || !globalThis.oidc.WebStorageStateStore) {
      throw contractError4("oidc-library-unavailable", "Core sign-in library did not load");
    }
    ...
    if (!response.ok) throw contractError4("oidc-discovery-failed", "Core authentication discovery failed with HTTP " + response.status);
    ...
    if (!authority || !metadata.resource || !metadata.x_client_id || !metadata.x_redirect_uri || !metadata.x_post_logout_redirect_uri) {
      throw contractError4("oidc-discovery-incomplete", "Core authentication discovery is incomplete");
    }
    if (new URL(metadata.resource, globalThis.location.origin).origin !== globalThis.location.origin) {
      throw contractError4("oidc-cross-origin-resource", "Core authentication resource must be same-origin");
    }
    if (new URL(authority, globalThis.location.origin).origin !== globalThis.location.origin) {
      throw contractError4("oidc-cross-origin-authority", "Core authorization server must be same-origin");
    }
    if (new URL(metadata.x_redirect_uri, globalThis.location.origin).href !== expectedCallback || new URL(metadata.x_post_logout_redirect_uri, globalThis.location.origin).href !== expectedCallback) {
      throw contractError4("oidc-callback-mismatch", "Core authentication callback does not match the portal contract");
    }
  ...
  async function loadCoreUserProfile(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var row = await requestJson4(fetchImpl, api.base + "/api/user/get.json?id=" + api.userId, options(api, USER_MAPPINGS));
  ...
  async function loadAccountPhone(api, fetchImpl) {
    ...
    var response = await requestJson4(fetchImpl, api.accountBase + "/api/account/list.json", options(api, {
    ...
    if (!account || positiveInteger6(account.id) !== api.accountId) return null;
    ...
    for (var index = 0; index < contacts.length; index += 1) {
      ...
      for (var entryIndex = 0; entryIndex < entries.length; entryIndex += 1) {
        ...
        if (text6(entry.type && entry.type.code) === "PHONE" && text6(entry.value)) return text6(entry.value);
      }
    }
    return null;
  }
  async function saveCoreUserProfile(input, context, fetchImpl = globalThis.fetch, explicitOrigin) {
    var api = requestContext2(context, explicitOrigin);
    var current = await requestJson4(fetchImpl, api.base + "/api/user/get.json?id=" + api.userId, options(api, USER_MAPPINGS));
    var entity = {
      email: text6(input && input.email),
      enabled: current.enabled !== false,
      fullname: text6(current.fullname),
      id: api.userId,
    ...
    await requestJson4(fetchImpl, api.base + "/api/user/save.json", options(api, { entities: [entity], mappings: USER_MAPPINGS }));
    var readback = await requestJson4(fetchImpl, api.base + "/api/user/get.json?id=" + api.userId, options(api, USER_MAPPINGS));
    return normalize(readback, api.userId);
  }
  function normalize(row, userId, phone) {
    if (!row || positiveInteger6(row.id) !== userId) throw error("profile-scope-mismatch", "Core User readback did not match the signed-in User");
    return {
      state: "ready",
      email: text6(row.email),
      phone: phone || null,
      prefs: {},
      optimistic: Number.isFinite(Number(row.optimistic)) ? Number(row.optimistic) : null,
  ...
  function requestContext2(context, explicitOrigin) {
    ...
    var state2 = context && context.state || {};
    var session = context && context.session || state2.session || {};
    var token = text6(session.accessToken || session.access_token);
    var userId = positiveInteger6(session.userId);
    ...
    var base = sameOriginBase4(config.coreApiBase || "/core", origin, "Core API base");
    var customer = context && context.account || state2.customerAccount || session.account || {};
    return {
      accountBase: sameOriginBase4(config.accountApiBase || "/core-acct", origin, "Core Account API base"),
      accountId: positiveInteger6(customer.id) || null,
      base,
      token: text6(session.tokenType || session.token_type || "Bearer") + " " + token,
      userId
    };
  }
  ...
  async function requestJson4(fetchImpl, url, requestOptions4) {
  ...
  function facts(property, contract, quote) {
    var rows = [
      { label: "Contract", value: contract ? "#" + contract.number + " \xB7 " + contract.plan : "Not under contract", action: contract ? "nav.go" : void 0, id: contract ? "proposals.list" : void 0 },
  ...
  function normalizePolicy(deployment, allowTestHosts) {
    ...
    var allowedOrigins = array2(own(deployment, "allowedOrigins", "deployment"), "deployment.allowedOrigins", 1).map(function(value, index) {
  ...
  function normalizeAuthored(payload, options2) {
    ...
    var services = array2(own(content, "services", "content"), "content.services", 1).map(function(service, index) {
    ...
    var faq = array2(own(content, "faq", "content"), "content.faq", 1).map(function(item, index) {
    ...
    var trust = array2(own(content, "trust", "content"), "content.trust", 0).map(function(fact3, index) {
      var path = "content.trust[" + index + "]";
      fact3 = object(fact3, path);
      return { key: "trust-" + index, icon: ["\u2605", "\u2696", "\u2714", "\u2B1A", "\u23F1"][index % 5], label: merge(own(fact3, "label", path), tags, path + ".label"), value: merge(own(fact3, "value", path), tags, path + ".value"), count: optionalMerge(own(fact3, "count", path), tags, path + ".count"), slot: null };
    });
    var reviews = array2(own(content, "reviews", "content"), "content.reviews", 0).map(function(review, index) {
    ...
    var normalized = {
      ...
      services,
      trust,
      how: normalizeItems(array2(own(content, "how", "content"), "content.how", 1), tags, "content.how", "title", "description"),
      proof: { title: merge(own(proof, "title", "content.proof"), tags, "content.proof.title"), items: normalizeItems(array2(own(proof, "items", "content.proof"), "content.proof.items", 1), tags, "content.proof.items", "title", "description") },
      pricing: { note: optionalMerge(own(pricing, "note", "content.pricing"), tags, "content.pricing.note"), rows: normalizePricingRows(array2(own(pricing, "rows", "content.pricing"), "content.pricing.rows", 0), tags) },
      area: { cities: array2(own(area, "cities", "content.area"), "content.area.cities", 1).map(function(city, index) {
  ...
  function normalizeFooter(footer, policy, tags) {
    ...
    return {
      ...
      hours: array2(own(footer, "hours", "content.footer"), "content.footer.hours", 0).map(function(row, index) {
      ...
      legal: array2(own(footer, "legal", "content.footer"), "content.footer.legal", 0).map(function(link, index) {
  ...
  function reviewsRegion(product) {
    ...
    if (view === "loading") {
      panel.appendChild(h("div", { "data-state": "loading", "aria-busy": "true" }, [skel2("height:64px;border-radius:14px"), skel2("height:64px;border-radius:14px;margin-top:10px")]));
      return panel;
    }
    if (view === "unavailable") {
  ...
  function SpaProductDetail() {
    ...
    var gate = routeStateBody({
      ...
      states: ["loading", "error", "unauthorized"],
      skeleton: function() {
        return h("div", { "class": "pd-grid", "data-state": "loading", "aria-busy": "true" }, [skel2("height:340px;border-radius:22px"), h("div", null, [skel2("height:26px;border-radius:9px"), skel2("height:80px;border-radius:12px;margin-top:12px"), skel2("height:48px;border-radius:14px;margin-top:16px")])]);
      },
  ...
  function createCoreAccountAdapter(options2 = {}) {
    var fetchImpl = options2.fetch || globalThis.fetch;
    if (typeof fetchImpl !== "function") throw contractError5("fetch-unavailable", "Core account adapter requires fetch");
    return {
      async load(moduleId, context) {
        if (moduleId !== "account") throw contractError5("unsupported-module", "Core account adapter cannot load " + moduleId);
  ...
  async function resolveCoreAccount(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    if (!accounts.length) throw contractError5("customer-not-linked", "No customer Account is linked to the signed-in Core User");
    if (accounts.length !== 1 || Number(accountReply.resultSize || accounts.length) > 1) {
      throw contractError5("customer-account-ambiguous", "The signed-in Core User must resolve to exactly one customer Account");
    }
    var account = accounts[0] || {};
    if (!positiveInteger7(account.id)) throw contractError5("invalid-customer-account", "Core Account response did not include an id");
    if (account.user && positiveInteger7(account.user.id) && Number(account.user.id) !== userId) {
      throw contractError5("customer-scope-mismatch", "Core Account user does not match the authenticated Core User");
    }
    return {
      state: "ready",
  ...
  function selectOrganization(configuredCode, basicInfo) {
    ...
    if (authorized.length && !authorized.includes(selected)) {
      throw contractError5("organization-forbidden", "Configured portal organization is not authorized for the signed-in Core User");
    }
    return selected;
  }
  async function requestJson5(fetchImpl, url, options2) {
    var response = await fetchImpl(url, options2);
    if (!response || typeof response.ok !== "boolean") throw contractError5("invalid-response", "Core request returned an invalid response");
    if (!response.ok) {
      var code = response.status === 401 ? "session-expired" : response.status === 403 ? "customer-forbidden" : "core-request-failed";
      var error2 = contractError5(code, "Core request failed with HTTP " + response.status);
      error2.status = response.status;
      throw error2;
    }
    try {
      ...
      throw contractError5("invalid-response", "Core response was not valid JSON");
    }
  }
  function sameOriginBase5(value, origin, label) {
    if (!origin) throw contractError5("origin-required", label + " requires a browser origin");
    var target = new URL(String(value || ""), origin);
    if (target.origin !== new URL(origin).origin) throw contractError5("cross-origin-service", label + " must be same-origin");
    return target.href.replace(/\/+$/, "");
  }
  ...
  var SUBSCRIPTION_MAPPINGS = [
    ...
    { key: "id", mappings: REF_MAPPINGS3, name: "order", type: "identifier" },
    { mappings: REF_MAPPINGS3, name: "states", type: "collection" }
  ];
  var ORDER_ITEM_MAPPINGS2 = [
    ...
    { name: "notes" },
    {
      ...
      mappings: REF_MAPPINGS3.concat([
        { name: "attributes" },
        { key: "id", mappings: REF_MAPPINGS3, name: "product", type: "identifier" }
      ]),
      name: "itemPrice",
      type: "identifier"
    },
  ...
  function createCorePlansAdapter(options2 = {}) {
    var fetchImpl = options2.fetch || globalThis.fetch;
    if (typeof fetchImpl !== "function") throw contractError6("fetch-unavailable", "Core plans adapter requires fetch");
    return {
      load(moduleId, context) {
        if (moduleId !== "plan") throw contractError6("unsupported-module", "Core plans adapter cannot load " + moduleId);
  ...
  async function loadCorePlans(context, fetchImpl = globalThis.fetch, explicitOrigin) {
    ...
    var accountId = positiveInteger8(api.customer && api.customer.id);
  ...
  async function loadEnrollments(api, accountId, fetchImpl) {
    var response = await requestJson6(fetchImpl, api.serviceBase + "/api/project/list.json", requestOptions3(api, {
      filters: [{ type: "STRING", operator: "=", property: "type.code", value: ENROLLMENT_TYPE }],
      mappings: ENROLLMENT_MAPPINGS,
      offset: 0,
      pageSize: positiveInteger8(api.config.plansPageSize) || 100
    }));
    var rows = Array.isArray(response && response.result) ? response.result : [];
    return rows.filter(function(row) {
      return attributeNumber2(row, "CUSTOMER_ACCOUNT") === accountId;
    }).map(normalizeEnrollment);
  }
  function normalizeEnrollment(row) {
    var id = positiveInteger8(row && row.id);
    if (!id) throw contractError6("invalid-plan", "Core plan enrollment response did not include an id");
    var state2 = firstState(row);
    var status = ENROLLMENT_STATUS[state2];
    if (!status)
      throw contractError6(
        "plan-status-unmapped",
        "Workflow state " + (state2 || "(none)") + " has no approved customer status for a package plan"
      );
    var total = attributeNumber2(row, "CREDITS_TOTAL");
    var used = attributeNumber2(row, "CREDITS_USED");
    var remaining = total != null && used != null ? Math.max(total - used, 0) : null;
    var sourceOrder = attributeNumber2(row, "SOURCE_ORDER");
    return {
      ...
      status,
      title: localizedName5(row.nls) || "Package",
      totalUses: total
    };
  }
  async function loadMemberships(api, accountId, fetchImpl) {
    var response = await requestJson6(fetchImpl, api.billBase + "/api/subscription/list.json", requestOptions3(api, {
      filters: [{ type: "INTEGER", operator: "=", property: "account.id", value: String(accountId) }],
  ...
  function currencyId(line) {
    ...
    return entry ? positiveInteger8(entry.value) : 0;
  }
  function intervalUnitId(line) {
    var entry = priceAttributeEntry(line, "INTERVAL");
    return entry ? positiveInteger8(entry.unit) : 0;
  }
  async function codesById(api, base, endpoint, ids, fetchImpl) {
    if (!ids.length) return {};
    var response = await requestJson6(fetchImpl, base + "/api/" + endpoint + "/list.json", requestOptions3(api, {
  ...
  async function membershipLines(api, subscriptions, fetchImpl) {
    ...
    if (!orderIds.length) return {};
    var response = await requestJson6(fetchImpl, api.billBase + "/api/order-item/list.json", requestOptions3(api, {
    ...
    rows.forEach(function(row) {
      ...
      if (orderIds.indexOf(positiveInteger8(row.order && row.order.id)) >= 0 && !byOrder[key]) byOrder[key] = row;
    });
    return byOrder;
  }
  function recurringPrice(line, labels) {
    ...
    return formatMoney3(Number(unitPrice.value), labels.currency) + suffix;
  }
  function normalizeMembership(row, line, labels) {
    var id = positiveInteger8(row && row.id);
    if (!id) throw contractError6("invalid-plan", "Core subscription response did not include an id");
    var state2 = firstState(row);
  ...
  function planContext(context, explicitOrigin) {
    ...
    return {
      ...
      organization,
      serviceBase: sameOriginBase6(config.serviceApiBase || "/core-svc", origin, "Core Service API base")
    };
  }
  ...
  async function requestJson6(fetchImpl, url, options2) {
    var response = await fetchImpl(url, options2);
    if (response.status === 401) throw contractError6("session-expired", "Core rejected the portal session");
    if (response.status === 403) throw contractError6("customer-forbidden", "Core refused the plan read for this customer");
    if (!response.ok) throw contractError6("plans-unavailable", "Core plan read failed with HTTP " + response.status);
    return response.json();
  }
  ...
  function orderId(row) {
    return positiveInteger8(row && row.order && row.order.id);
  }
  function membershipTitle(line) {
    if (!line) return "Membership";
    var product = line.itemPrice && line.itemPrice.product;
    return localizedName5(product && product.nls) || localizedName5(line.itemPrice && line.itemPrice.nls) || "Membership";
  }
  function formatMoney3(amount, currency) {
  ...
  function normalizeProducts(raw) {
    if (raw.pimProducts) {
      return {
        ...
        models: clone3(raw.pimProductModels || []),
        reviews: clone3(raw.pimProductReviews || []),
        enrichment: clone3(raw.pimEnrichment || { models: "unavailable", reviews: "unavailable" }),
        source: "core-pim"
      };
    }
    return {
  ...
  function locationGroup(f, o) {
    ...
    if (src === "loading") {
      ...
      group.appendChild(h("div", null, [skel2("height:52px;border-radius:14px"), skel2("height:52px;border-radius:14px;margin-top:9px")]));
      return group;
    }
    if (src === "error") {
  ...
  function addonGroup(f, o) {
    ...
    if (src === "loading") {
      group.setAttribute("aria-busy", "true");
      group.appendChild(h("div", null, [skel2("height:58px;border-radius:14px"), skel2("height:58px;border-radius:14px;margin-top:9px")]));
      return group;
    }
    if (src === "error") {
