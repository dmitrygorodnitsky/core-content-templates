...
const blocksRoot = join(root, "blocks");
...
const blockJsonFiles = listFiles(blocksRoot, (file) => file.endsWith("block.json")).sort();
...
if (manifest) {
  ...
  for (const block of manifest.blocks) {
    ...
    if (block.path !== `blocks/${local.block.category}/${local.block.id}`) {
