...
const usage = () => `Usage:
node app-templates/landing-page/scripts/generate-cms-family.mjs \\
  --copy app-templates/landing-page/compositions/generated/<legacy-copy>.md \\
...
const main = () => {
  ...
  // Safety: the generator wipes args.out blindly. Refuse paths outside the
  // generated-output zone owned by this application.
  ...
  if (!relativeOut || relativeOut === "." || relativeOut.startsWith("..")) {
    throw new Error(
      `Refusing to wipe --out path outside ${distRoot}: ${args.out}`,
    );
  }
