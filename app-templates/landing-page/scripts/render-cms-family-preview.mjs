...
const usage = () => `Usage:
node app-templates/landing-page/scripts/render-cms-family-preview.mjs \\
...
const renderAssetPlaceholders = (source, values, locale) =>
  String(source || "").replace(/\$\{([A-Z0-9_]+)@([A-Z0-9_]+)\}/g, (_, code) =>
    valueToString(values[code], locale));
...
const main = () => {
  ...
  const rawCss = allTemplates
    .map((template) => renderAssetPlaceholders(template?.css, values, locale))
    .filter((value) => value.trim())
    .join("\n\n");
  const { imports: cssImports, body: css } = hoistCssImports(rawCss);
  const js = allTemplates
    .map((template) => renderAssetPlaceholders(template?.javascript, values, locale))
    .filter((value) => value.trim())
    .join("\n\n");
